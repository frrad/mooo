...
| DR-MAC-022 | 2026-09-24 | macOS 26.8.0 | Observed/dynamic, authorized debug copy | With the official client quit and no KakaoTalk process present, one logged-out debug run entered the QR-generate route once; its sanitized callback reported success, and the parser then observed HTTP 200 with a present response dictionary. The QR was cancelled without approval, scan, polling, or login. | High for the observed local call ordering and predicates; not evidence of server enrollment | No check-key getter callback was reached; preference isolation remains unresolved because two official-container preference files changed during the debug run |
...
DR-MAC-022 passed transfer review on 2026-09-24 as a sanitized dynamic call-order
observation. It may corroborate the QR-generate state machine's single-route,
HTTP-200/present-response success boundary, but it does not establish enrollment,
QR URL contents, check-key behavior, or preference isolation.
